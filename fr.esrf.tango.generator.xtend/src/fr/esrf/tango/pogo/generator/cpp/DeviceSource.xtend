...
class DeviceSource {
	...
	def generateDeviceSourceFile (PogoDeviceClass cls) '''
		...
		«cls.protectedArea("namespace_ending", "	/* LABVIEW COMMUNICATION BEGIN ID(namespace_ending)*/
	template<typename T>
	void "+cls.name+"::labViewRead(T* value) {
		if (!tcp_manager)
			return;
		string msg;
		tcp_manager->read(msg);
		Decoder::decode(msg, value);
	}
	
	template<typename T>
	void "+cls.name+"::labViewWrite(std::string msg, T value) {
		if (!tcp_manager)
			return;
		if (value) {
			msg += Encoder::encode(value);
			tcp_manager->write(msg);
		}
		else {
			msg += \"\\r\\n\";
			tcp_manager->write(msg);
	...
	template<typename T>
	void "+cls.name+"::labViewWrite1D(std::string msg, T value, int size) {
		if (!tcp_manager)
			return;
		if (value) {
			msg += Encoder::encode1d(value,size);
			tcp_manager->write(msg);
		}
		else {
			msg += \"\\r\\n\";
	...
	template<typename T>
	void "+cls.name+"::labViewWrite2D(std::string msg, T value, int sizex, int sizey) {
		if (!tcp_manager)
			return;
		if (value) {
			msg += Encoder::encode2d(value, sizex, sizey);
			tcp_manager->write(msg);
		}
		else {
			msg += \"\\r\\n\";
	...
	void "+cls.name+"::updateState() {
		if (!tcp_manager) {
			set_state(Tango::INIT); 
			return;
		}
		int currLvState = 0;
		labViewWrite<Tango::DevString*>(\"SYSTem:ERRor?\");
		Tango::DevString* vi_state = new Tango::DevString[1];
		labViewRead(vi_state);
		currLvState = atoi(vi_state[0]);
		delete[](vi_state);
		switch (currLvState) {
		case 0:
			set_state(Tango::RUNNING);
			set_status(\"No Errors.\");
			return;
		default:
			set_state(Tango::FAULT);
			set_status(\"Some error(s) occurred!\");
			checkForErrorsAndUpdate = false;
			return;
		}
	}

	/* LABVIEW COMMUNICATION END */"+"//Additional Methods", false)»
		} //	namespace
	'''
	...
	def fileHeader (PogoDeviceClass cls) '''
		«cls.protectedArea(".cpp",
			...
			"bool checkForErrorsAndUpdate = false;", false)»
	'''
	...
	def constructors(PogoDeviceClass cls) '''
		...
		void «cls.name»::delete_device()
		{
			...
			«cls.protectedArea("delete_device", "	/* LABVIEW COMMUNICATION BEGIN ID(delete_device)*/\ndelete tcp_manager;\ntcp_manager = 0;\n	/* LABVIEW COMMUNICATION END */"
				+ "\n//Delete device allocated objects", false)»
	...
	def initDeviceMethod(PogoDeviceClass cls) '''
		...
		void «cls.name»::init_device()
		{
			...
			«ENDIF»
			«cls.protectedArea("init_device", "	/* LABVIEW COMMUNICATION BEGIN ID(init_device)*/
\ncheckForErrorsAndUpdate = true;\nif (tcp_manager)\ndelete tcp_manager;\n\n	tcp_manager = new AsioTCPManager(ip, port);
	\nif (!tcp_manager->openSocket()) {\ndelete tcp_manager;\ntcp_manager = 0;\n}\n\n//*attr_ViPath_read = \"\";\nif(checkForErrorsAndUpdate)\n\tupdateState();\n	
/* LABVIEW COMMUNICATION END */\n" 
+"//Initialize device", false)»
	...
	def utilsMethods(PogoDeviceClass cls) '''
		...
		void «cls.name»::always_executed_hook()
		{
			...
			«ENDIF»
			«cls.protectedArea("always_executed_hook"," /* LABVIEW COMMUNICATION BEGIN ID(always_executed_hook)*/\nif(checkForErrorsAndUpdate)\n\tupdateState();\n	/* LABVIEW COMMUNICATION END */\n"+"//code always executed before all requests", false)»
		}
